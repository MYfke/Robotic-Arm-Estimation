...
import lib.multiviews.cameras as cameras
from lib.multiviews.body import HumanBody
from lib.utils.transforms import get_affine_transform, affine_transform_pts


def infer(unary, pairwise, body, config):
    """
    Args:
        unary: a list of unary terms for all JOINTS  所有 JOINTS 的一元术语列表
        pairwise: a list of pairwise terms of all EDGES  所有边缘的成对项列表
        body: tree structure human body  树状结构人体
    Returns:
        pose3d_as_cube_idx: 3d pose as cube index  作为立方体索引的 3d 姿势
    """

    skeleton = body.skeleton
    skeleton_sorted_by_level = body.skeleton_sorted_by_level
    root_idx = config.DATASET.ROOTIDX      # 根关节点的等级
